    ...
    var body: some View {
        VStack (spacing: 10) {
            ...
            if(settings.isBackgroundImageEnabled) {
                DefaultSettingsRow(title: "Background Blur", subtitle: "Blur Value", content: {
                    Text(String(format: "%.0f", settings.backgroundBlur))
                    Slider(
        ...
        .animation(.spring(duration: settings.animationDelay), value: settings.isBackgroundImageEnabled)
    }
}
